...
Ufront= @(x,y,timet)  exp(-2*timet)*sin(x)*sin(y);   %Función U
Qfront= @(x,y,timet)  exp(-2*timet)*cos(x)*sin(y)*x+exp(-2*timet)*sin(x)*cos(y)*y; 


%CONDICIONES INICIALES
Ucero= @(x,y) sin(x)*sin(y);
Qcero= @(x,y) cos(x)*sin(y)*x+sin(x)*cos(y)*y;
...
k=input('Digite 0 ó 2 para las condiciones de frontera Tipo Dirichlet y Mixto :'); 


Aaux=bigC+Dt*thetau*bigH;
bigGaux=Dt*thetaq*bigG;
...
for m=1:PasosM
    ...
    d=(bigC-Dt*(1-thetau)*bigH)*bigU+Dt*(1-thetaq)*bigG*bigQ;
...
%PROBLEMA MIXTO-> K=2 SI CONOCEMOS UNA PARTE DE U Y Q SOBRE LA FRONTERA. 
    ...
    if k==2
        ...
        A1=[-Aaux(:,1:r) bigGaux(:,r+1:N+NI)];
        e=A1*T;
        
        A=[-bigGaux(:,1:r) Aaux(:,r+1:N+NI)];
        Y=d+e;
        X=pinv(A)*Y;
