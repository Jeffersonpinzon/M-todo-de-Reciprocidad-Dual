...
N=input('Numero de nodos frontera:'); %Número de puntos sobre la frontera que vamos a considerar
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %PROBLEMA TIPO DIRICHLET-> K=0 SI CONOCEMOS TODOS LOS VALORES DE U SOBRE LA FRONTERA.
        ...
        Ufrontera=Ufrontaux;%valor conocido
        Ufrontaux=[Ufrontaux,zeros(1,NI)];
        bigU1=Ufrontaux';
        A1=[-A(:,1:N) bigG(:,N+1:N+NI)];
        A2=[-bigG(:,1:N) A(:,N+1:N+NI)];
        Y0=A1*bigU1;
        X0=A2\Y0; 
        Qfrontera=X0(1:N); %VALORES ENCONTRADOS
        Uint=X0(N+1:N+NI); %VALORES ENCONTRADOS 
    ...
    if k==1
        ...
        Qfrontera=Qfrontaux; %valor conocido
        Qfrontaux=[Qfrontaux,zeros(1,NI)];
        bigQ=Qfrontaux';
        Y1=bigG*bigQ;
        %X=pinv(A)*Y; 
        X1=A\Y1; 
        bigU=X1; %VALORES ENCONTRADOS 
    ...
    %Una buena aproximación es N=50 Y 25 nodos Ufron conocidos
