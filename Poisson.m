...
Ufront= @(x,y)  exp(x)*sin(y)+x^3/6+y^3/6;  %Función U
Qfront= @(x,y)  (exp(x)*sin(y)+x^2/2)*x+(exp(x)*cos(y)+y^2/2)*y; %Derivada normal de U sobre la frontera. Este ejemplo es suponiendo que la fontera es una circunferencia unitaria. Si la frontera cambia esta formula cambia pues el vector normal cambia
...
N=input('Numero de nodos frontera:'); %Número de puntos sobre la frontera que vamos a considerar 50
...
for i=1:10
    xint(i)=0.8*cos(2*pi*(i-1)/10);
    yint(i)=0.6*sin(2*pi*(i-1)/10);
    Pint(i,:)= [xint(i),yint(i)];
end

for i=1:10
    xint(i)=0.6*cos(2*pi*(i-1)/10);
    yint(i)=0.4*sin(2*pi*(i-1)/10);
    Pint(10+i,:)= [xint(i),yint(i)];
end

for i=1:10
    xint(i)=0.4*cos(2*pi*(i-1)/10);
...
%CÁLCULO DEL VECTOR \alpha

%Cálculo del verctor b
for i=1:N+NI
   vecb(i)=funb(hatP(i,1),hatP(i,2));
end
vecb=vecb';%Para volverlo columna
...
k=input('Digite 0 ó 1 para las condiciones de frontera Tipo Dirichlet y Mixta :');
...
if k==2
    ...
    A1=[H(:,1:r) -G(:,r+1:N)]; %VALORES CONOCIDOS
    A2=[G(:,1:r) -H(:,r+1:N)]; %VALORES DESCONOCIDOS
    Y2=A1*T-vecd;
    X2=A2\Y2;
...
%VAMOS A USAR LOS VALORES EXACTOS CONOCIDOS PARA COMPARAR. EN ESTE CASO LO
%PODEMOS HACER POR QUE CONOCEMOS SOLUCIÓN ANALITICA DEL PROBLEMA.
for i=1:N
   Ufrontexac(i)=Ufront(P(i,1),P(i,2));
   Qfrontexac(i)=Qfront(P(i,1),P(i,2));
end

ErrorU=norm(Ufrontera'-Ufrontexac)/norm(Ufrontexac)
...
ErrorUint=norm(Uint'-UintExac)/norm(UintExac)
...
% %GRÁFICA
% Se debe crear una malla uniformemente espaciada. 
%Los valores desconocido se calculan interpolando los valores conocidos. 
